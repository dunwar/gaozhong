/**
 * Phase 1.1 v5 — 新架构：阶段1只切题 → 阶段2裁切图红笔检测+判错
 * 
...
const OUTPUT_DIR = join(__dirname, 'output', 'scanner-test-v5b');
...
const STAGE1_FULL_BUT_QUESTIONS_ONLY = `你是高中试卷分析专家。分析这张已批改的试卷图片。

【任务 — 题目区域识别】
识别每道题的完整边界。题目类型：choice / fill_blank / reading / dictation / translation / writing

⚠️ 关键规则：
1. 整页所有题号(21,22,23...或1,2,3...)，每个题号=一道独立题目
2. 两栏布局逐栏逐行识别
3. bbox 左起题号位置，右至题目正文/选项结束处（约在页面2/3宽度处）
4. ⚠️ 不要包含试卷右边缘整齐排列的印刷字母列（那是答案键，不是题目内容）
5. 不要遗漏：跨页续题（只有选项没题干的）也独立成题

直接输出纯 JSON（不要 markdown）：
{
  "questions": [
    {"questionNumber": 21, "bbox": {"x": 40, "y": 80, "w": 550, "h": 120}, "questionType": "choice"}
  ]
}`;
...
async function stage1_detectQuestions(originalPath) {
  ...
  const result = await kimiVision([
    ...
    { role: 'user', content: [{ type: 'text', text: STAGE1_FULL_BUT_QUESTIONS_ONLY }, { type: 'image_url', image_url: { url: b64, detail: 'auto' } }] }
...
async function main() {
  ...
  if (!sessionId) {
    console.log('用法: node test-scanner-v5.mjs <sessionId> [pageIndex]');
    process.exit(1);
  }
  ...
  console.log('╔══════════════════════════════════════════════════╗');
  console.log('║   v5 Scanner — 阶段1只切题 + 阶段2裁切判错     ║');
  console.log('╚══════════════════════════════════════════════════╝');
