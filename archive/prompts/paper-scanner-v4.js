 ...
 * 改进：
 *   - 新增印刷红字 vs 手写红笔区分规则
 *   - 图片压缩降质加速（1200px/55q）+ detail:auto
 */

export const PAPER_SCANNER_VERSION = 'v4.2';
...
❌ 题目本身就包含红色印刷（如下划线、边框、标题）→ 不是批改，忽略
❌ 红笔痕迹极其细微（仅一个小点、短划线<2mm）→ 可能不是批改，跳过

═══════════════════════════════════════
【⚠️ 印刷红字 vs 手写红笔 — 关键区分】
═══════════════════════════════════════
试卷上可能同时存在印刷的红色文字（标题、边框、下划线）和
手写的红色批改标记。区分方法：
- 印刷红字：字体规范、位置固定、颜色一致、通常在页眉/页脚/题头
- 手写红笔：笔迹不规整、有轻重变化、颜色深浅不一、通常写在题目旁边或选项上

⚠️ 只有手写红笔才算批改。印刷红字一律忽略。
