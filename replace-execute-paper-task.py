#!/usr/bin/env python3
"""
自动替换 executePaperTask 函数为 VL 统一分析版本
"""
import re
import sys

# 新函数代码
NEW_FUNCTION = '''async function executePaperTask(task) {
  const { id, input } = task;
  try {
...
def replace_function(file_path):
    ...
    # 查找旧函数：从 "async function executePaperTask(task) {" 到下一个 "async function" 或 "// ==========" 
    pattern = r'(async function executePaperTask\(task\) \{.*?)(\n// ========== V2 AI 学习指导执行器 ==========)'
    match = re.search(pattern, content, re.DOTALL)
    
    if not match:
        print("❌ 未找到 executePaperTask 函数")
        return False
    
    old_func = match.group(1)
    print(f"✅ 找到旧函数（{len(old_func)} 字符）")
    ...
    print(f"✅ 替换完成（{len(NEW_FUNCTION)} 字符）")
    return True

if __name__ == '__main__':
    if len(sys.argv) < 2:
        file_path = 'api-server.js'
    else:
        file_path = sys.argv[1]
    
    if replace_function(file_path):
        print("✅ 成功！请重启 API 服务：bash /app/data/start-api.sh")
    else:
        print("❌ 失败")
        sys.exit(1)
